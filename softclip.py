class SoftClip:
    def __init__(self, contig, direction, position, contig_length, n_clipped_reads, n_multi_mapped, n_double_clipped, clip_consensus):
        self.contig = contig
    ...
    def __repr__(self):
        string = f"{self.sv_type if self.sv_type else 'remains'} {self.contig}:{self.position} ({self.direction}) CONTIG_LEN: {self.contig_length}, #CLIP_READS:{self.n_clipped_reads}, SEQ:{self.clip_consensus_sequence}, CLIPSEQLEN:{len(self.clip_consensus_sequence)}, N_\
